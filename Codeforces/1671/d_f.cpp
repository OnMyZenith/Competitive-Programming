...
void solve() {
    int n, x; re(n, x); rv(n, a);
    ll mn = *min_element(all(a));
    ll mx = *max_element(all(a));

    ll cost = 0;
    f0r(i, n - 1) cost += abs(a[i + 1] - a[i]);

    ll lcost = 0, rcost = 0;
    if(mn > 1) lcost = min(a.front(), a.back()) - 1;
    if(mx < x) rcost = x - max(a.front(), a.back());
    // dbg(cost, lcost, rcost);
    for (int i = 0; i < n - 1; i++) {
        ckmin(lcost, a[i] - 1 + a[i + 1] - 1 - abs(a[i + 1] - a[i]));
        ckmin(rcost, abs(x - a[i]) + abs(x - a[i + 1] - abs(a[i + 1] - a[i])));
    }
    cout << cost + lcost + rcost << '\n';
}
