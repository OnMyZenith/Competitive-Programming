...
void solve() {
    int n; re(n); rv(n,a); sort(all(a),[](pi x,pi y){return abs(x.ff-x.ss) < abs(y.ff-y.ss);});
    vi res(n); set<int> poss; f0r(i,n)poss.ins(i+1);
    f0r(i,n) {auto it = poss.lb(a[i].ff); res[i] = *it; poss.erase(it);}
    f0r(i,n) pr(a[i].ff," ",a[i].ss," ",res[i],'\n');
}
