...
template <class T> struct maxsegtree {
...
int main() {
    ...
    minsegtree<int> mn; mn.build(b);
    maxsegtree<int> mx; mx.build(b);

    vector<int> ans(n);
    ans[n - 1] = 1; int maxm = a[n - 1];
    for (int i = 0; i < n - 1; i++, ans[n - 1]++) {
        if(a[i] * 2 < maxm) break;
        ckmax(maxm, a[i]);
    }
    for (int i = n - 2; i >= 0; i--) {
    ...
    for (int i = 0; i < n; i++) {
        int j = lt(0, n - 2, [&](int k)->bool{
            int minv = mn.q(i, i + k);
            int maxv = mx.q(i, i + k);
            if(minv * 2 < maxv) return false;
            return true;
        });
        if(j == n - 1) cout << -1 << " \n"[i == n - 1];
        else cout << j + 1 << " \n"[i == n - 1];
