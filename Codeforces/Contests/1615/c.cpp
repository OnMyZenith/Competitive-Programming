...
void solve() {
    int n; re(n); str s,t; re(s,t);
    if(s==t){ps(0);return;}

    bool f1d=0; f0r(i,n)if(s[i]==t[i]) {if(s[i]=='1') f1d=1;}
    dbg(f1d);
    int f1=0,f0=0;
    f0r(i,n)if(s[i]!=t[i]) {if(s[i]=='1') f1++;else f0++;}
    if(f1==0&&(!f1d)) {ps(-1);return;}

    int ans = iINF;
    if(((n-(f1+f0))&1)&&f1d) ckmin(ans,n-(f1+f0));
    if(((f1+f0)%2) == 0 && f1>1) ckmin(ans,f1+f0);
    dbg(f1,f0);
    if(ans==iINF) {ps(-1);return;}
    ps(ans);
