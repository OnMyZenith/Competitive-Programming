...
void solve() {
    int n; re(n); rv(n,a); vl f(n+1);
    each(i,a) f[i]++;
    // dbg(f);
    vl b;
    f0r(i,n+1) if(f[i]) b.pb(f[i]);
    int m = sz(b);
    vl pre(m), preSuff(m);
    f0r(i,m) pre[i] = (i?pre[i-1]:0LL) + b[i];
    reverse(all(b));
    f0r(i,m) preSuff[i] = (i?preSuff[i-1]:0LL) + b[i];
    reverse(all(b));

    // f0rd(i,m-1) suff[i] = (i!=m-1?suff[i+1]:0) + b[i];

    int mx = log_2_ceil(n) + 1;
    // dbg(b);
    ll best = next_pow_2(pre[m-1])-pre[m-1] + 2;
    f1r(i,-1,mx){
        f1r(j,-1,mx){
            ll l, r; int lidx, ridx; ll tmp = 0;
            
            if(i!=-1){
                l = 1LL << i;
                lidx = lstTrue(0,m-1,[&](int x)->bool{return pre[x]<=l;});
            }else{
                lidx = -1;
            }
            ...
            if(j!=-1){
                r = 1LL << j;
                ridx = m - 1 - lstTrue(0,m-1,[&](int x)->bool{return preSuff[x]<=r;});
            }else{
                ridx = m;
            }
            ...
            if(lidx!=-1){
                tmp += (ll) next_pow_2(pre[lidx]) - pre[lidx];
            }else{
                tmp++;
            }

            if(ridx!=m){
                tmp +=next_pow_2(pre[m-1]-(ridx?pre[ridx-1]:0)) - pre[m-1]+(ridx?pre[ridx-1]:0);
            }else{
                tmp++;
            }

            // dbg(best,lidx,ridx);
            // tmp = (ll) next_pow_2(pre[lidx]) - pre[lidx] + next_pow_2(pre[m-1]-pre[ridx-1]) - pre[m-1]+pre[ridx-1];

            if(ridx>lidx+1) tmp+= (ll)next_pow_2(pre[ridx-1]-(lidx!=-1?pre[lidx]:0)) - pre[ridx-1]+(lidx!=-1?pre[lidx]:0);
            else tmp++;
            ckmin(best,tmp);
        }
    }
