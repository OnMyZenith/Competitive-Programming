...
void solve() {
    int n; re(n); a.rsz(n); f0r(i,n){ re(a[i].ff); a[i]={a[i].ff,i};}  str s; re(s); l.clear(); d.clear();
    f0r(i,n)if(s[i]=='1')l.pb(a[i]); else d.pb(a[i]);
    int cnt = sz(d); sor(l); sor(d);
    vl res(n); 
    f0r(i,sz(l)) res[l[i].ss] =++cnt;
    cnt = sz(d); f0rd(i,sz(d)-1) res[d[i].ss] = cnt--;
    // ps(res);
    ao(res,n);
