...
void solve() {
    ll n, k; re(n,k); rv((int)n,a); soR(a); ll sm = 0; f0r(i,n) sm+=a[i];
    ll r = sm - k;

    vl pre(n); f0r(i,n-1) pre[i+1]= pre[i]+a[i];
    ...
    dbg(r);
    f0r(i,n){
        ll res = fstTrue(0LL,(ll)iINF,[&](ll x){
            return (pre[i] + x - (i)*(a[n-1]-x))>=r;
        });
        dbg(res);
        // if((pre[i] + res) < r) res=iINF;
        dbg(i,res);
        ckmin(ans,res+i);
    }
    ps(ans);
