...
void solve() {
    ll n; re(n); str s; re(s); str temp = s; reverse(all(temp));
    str best  = s+temp;
    f0r(i,n){
        str r1 = s.substr(0,i+1);
        str r2 = r1; reverse(all(r2));
        str r3 = r1+r2;
        if(!lexicographical_compare(all(best),all(r3))) best = r3;
    }
    // ll i = 0;
    // while(i<n-1&&s[i]>s[i+1])i++;
    // str r = s.substr(0,i+1);
    // pr(r);
    // reverse(all(r));
    // pr(r);
    // ps();
