...
#define f1r(i, l, r) for (ll i = (l); i <= (r); ++i)
#define f0rd(i, n) for (ll i = (n); i >= 0; --i)
#define f1rd(i, l, r) for (ll i = (l); i >= (r); --i)
...
void solve() {
    int n; ll l,k; re(n,l,k);
    rv(n,d); rv(n,a); d.pb(l);
    vvl dp(n+2,vl(k+1, lINF)); // minimum time = dp[followingThisPost][SkippedTheseManyPosts_UPTO_NOW]
    dp[1][0] = 0;

    f1r(i,2,n+1){
        f1r(j,0,k){
            f1r(lj,0,min(j,i-2)){ // last jump
                ckmin(dp[i][j], dp[i-1-lj][j-lj] + (d[i-1] - d[(i-1-lj)-1])*a[(i-1-lj)-1]);
            }
        }
    }
    ps(*min_element(all(dp[n+1])));
