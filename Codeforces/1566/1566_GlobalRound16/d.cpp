...
void solve() {
    ...
    sort(all(eye), comp);
    vi per_seat(n*m);// person_seat[1st person] = seat no.
    f0r(i,n*m){
        per_seat[eye[i].ss] = i;
    }
    ...
    int in = 0;
    f0r(i,n*m){
        in += s[person_seat[i].ff].order_of_key(person_seat[i].ss);
        s[person_seat[i].ff].insert(person_seat[i].ss);
    }
    cout << in << '\n';
}
