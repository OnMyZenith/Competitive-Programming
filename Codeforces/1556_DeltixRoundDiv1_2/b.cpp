...
void solve() {
    ...
    if(sz(eve)>sz(odd)){
        int j = 0, k = 0;
        f0r(i,n){
            if(i&1)ans += max(odd[j++] - i, 0);
            else ans += max(eve[k++] - i, 0);
        }
    }
    else if (sz(eve)<sz(odd)){
        int j = 0, k = 0;
        f0r(i,n){
            if(i&1)ans += max(eve[j++] - i, 0);
            else ans += max(odd[k++] - i, 0);
        }
    }else{
        int j = 0, k = 0;
        f0r(i,n){
            if(i&1)ans1 += max(odd[j++] - i, 0);
            else ans1 += max(eve[k++] - i, 0);
        }
        j = 0, k = 0;
        f0r(i,n){
            if(i&1)ans2 += max(eve[j++] - i, 0);
            else ans2 += max(odd[k++] - i, 0);
        }
        ans = (min(ans1, ans2));
    }
    cout << ans << '\n';
