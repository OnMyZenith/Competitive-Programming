...
void solve() {
    ...
    vector<int> tmp(n); iota(tmp.begin(), tmp.end(), 0);
    set<int> row_remain(tmp.begin(), tmp.end()), col_remain; col_remain = row_remain;
    int cnt = 0;
    for (int i = q - 1; i >= 0; i--) {
        auto &[r, c] = all[i];
        bool f = erase(row_remain, r);
        bool g = erase(col_remain, c);
        if(row_remain.empty()) col_remain.clear();
        if(col_remain.empty()) row_remain.clear();
        cnt += f | g;
    }
    mi ans = pow(mi(k), cnt);
    cout << ans.v << '\n';
