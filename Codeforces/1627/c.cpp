...
void solve() {
    int n; re(n); vvi g(n); bool poss = 1; int l = -1; map<pi,pi> edges;
    f0r(i,n-1){
        int x,y; re(x,y); x--,y--;
        g[x].pb(y); g[y].pb(x);
        if(g[x].size()>2) poss = 0;
        if(g[y].size()>2) poss = 0;
        // if(g[x].size()==1) l = x;
        // if(g[y].size()==1) l = y;
        if(x>y) swap(x,y);
        edges[{x,y}] = {i,-1};
    }
    f0r(i,n)if(g[i].size()==1){l=i;break;}
    if(!poss){ps(-1);return;}
    dbg(l);
    auto dfs = y_combinator([&](auto self, int v, int p, int w)->void{
        each(u,g[v]){
            if(u!=p){
                if(u>v) edges[{v,u}].ss = w;
                else edges[{u,v}].ss = w;
                self(u,v,(w==2?5:2));
            }
        }
