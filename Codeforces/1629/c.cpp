...
void solve() {
    int n; re(n); rv(n,a); b.clear();
    int M = *max_element(all(a));
    vvi c(M+2); f0r(i,n) c[a[i]].pb(i);
    int curr = 0;
    while(curr<n){
        int nCurr = curr;
        f0r(i,M+2){
            auto it = lb(all(c[i]),curr);
            if(it==c[i].end()){b.pb(i); break;}
            else ckmax(nCurr,*it);
        }
        curr = nCurr + 1;
    }    
...
//     int n; re(n); rv(n,a); b.clear();
//     int M = *max_element(all(a));
//     set<int> s,r, tmp; f0r(i,M+2) s.ins(i);
//     tmp = s;
//     vi mex(n);
//     f0rd(i,n-1){
//         s.erase(a[i]);
...
//     dbg(mex);
//     y_combinator([&](auto self, int i, int mx)->void{
//         // if(i>=n) return;
//         b.pb(mx); r = tmp;
//         r.erase(a[i]); i++;
//         while(*r.begin()!=mx){
