...
void solve() {
    int n; re(n); rv(n,a); vb have(n+1); vl excess; f0r(i,n)if(a[i]<n+1 && (!have[a[i]])) have[a[i]]=1; else excess.pb(a[i]);
    vl need; f0r(i,n)if(!have[i+1]) need.pb(i+1);
    soR(need); soR(excess); int x = (int)sz(excess);
    // dbg(need,excess);
    f0r(i,x){
        if(excess[i]<=2*need[i]){ps(-1); return;}
    }
    ps(x);
