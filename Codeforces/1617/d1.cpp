    ...
    */
    ...
    void solve() {
        int n; re(n); vi a(n,0); vvi v(2,vi{}); ord_set<int> s; f0r(i,n)s.ins(i);
        while(!s.empty()){
            int i=0;
            bool p=moreCrewmates(*s.find_by_order(i),*s.find_by_order(i+1),*s.find_by_order(i+2)); i++;
            while(p==moreCrewmates(*s.find_by_order(i),*s.find_by_order(i+1),*s.find_by_order(i+2))){ i++; }
            v[p].pb(*s.find_by_order(i-1));
            v[!p].pb(*s.find_by_order(i+2));
            v[moreCrewmates(*s.find_by_order(i),*s.find_by_order(i-1),*s.find_by_order(i+2))].pb(i);
            v[moreCrewmates(*s.find_by_order(i+1),*s.find_by_order(i-1),*s.find_by_order(i+2))].pb(i+1);
            erase(s,i-1);
            erase(s,i);
            erase(s,i+1);
            erase(s,i+2);
