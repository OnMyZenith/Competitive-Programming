...
void solve() {
    int l,r; re(l,r); int len=r-l+1;
    int cost = iINF;
    f0r(i,20){
        ckmin(cost,((r-c[i][r]) - (l-1-c[i][l-1])));
    }
    ps(cost);
    // int best = -1;
    ...
    // f0r(i,30){
    //     int cnt=0;
    //     if((1<<i)<=l&&((1<<(i+1))-1)>=r) cnt= len;
    //     else if((1<<i)>=l&&((1<<(i+1))-1)<=r) cnt= 1<<i;
    //     else if((1<<i)>=l&&(1<<i)<=r) cnt=r-(1<<i)+1;
    //     else if(((1<<(i+1))-1)>=l&&((1<<(i+1))-1)<=r) cnt = ((1<<(i+1))-1)-l+1;
    //     else continue;
    //     dbg(cnt);
    //     if(ckmax(best,cnt))dbg((1<<i),((1<<(i+1))-1));
    //     ckmax(best,cnt);
    // }
    // ckmax(best,len/2);
    // if((l&1)&&(r&1)) ckmax(best,len/2+1);
    // if((!(l&1))&&(!(r&1))) ckmax(best,len/2+1);
    // ps((len - best));
}
