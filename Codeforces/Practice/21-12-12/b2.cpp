...
void solve() {
    int n,k;re(n,k);rv(n,a);b.ass(n,0); hash_map<int,vi> h;
    f0r(i,n)h[a[i]].pb(i); int mn=iINF; each(i,h) ckmin(mn,max(k,(int)sz(i.ss)));int cnt = mn/k; mn=mn-mn%k;
    
    each(i,h){
        if(sz(i.ss)<k)continue;
        f0r(j,mn){
            b[i.ss[j]] = j%k+1;
        }
    }
