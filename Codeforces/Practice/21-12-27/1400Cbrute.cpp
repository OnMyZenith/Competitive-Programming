...
void solve() {
    str s; int x; re(s, x);int n = sz(s); bool f = 0; str r(n, '1');
    f0r(i, n) {
        if( s[i] == '0'){
            if(i + x < n) r[i + x] = '0';
            if(i - x >= 0) r[i - x] = '0';
    ...
    f0r(i, n) {
        bool one = false;
        one = one || (i - x >= 0 && r[i - x] == '1');
        one = one || (i + x < n && r[i + x] == '1');
        ...
        // if (((i + x < n && r[i + x] == '1') || (i - x >= 0 && r[i - x] == '1')) && s[i] != '1') f = 1;
        // else if ((i + x < n && r[i + x] == '0') && (i - x >= 0 && r[i - x] == '0') && s[i] == '1') f = 1;
        // else if (i - x < 0 && i + x < n && r[i + x] == '0' && s[i] == '1') f = 1;
        // else if (i + x >= n && i - x >= 0 && r[i - x] == '0' && s[i] == '1') f = 1;
    }
    // if (f) { ps(-1); return;}
    ps(r);
}
