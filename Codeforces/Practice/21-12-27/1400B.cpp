...
void solve() {
    ll P,F,CS,CW,S,W; re(P,F,CS,CW,S,W); if(S>W) swap(W,S),swap(CS,CW);
    

    // s_m = min(p/s,cs); cs -= s_m; p -= s*s_m;
    ll ans=0;
    f0r(s_m,min(P/S,CS)+1){
        ll w_m = 0, s_f = 0, w_f = 0, p=P,f=F,cs=CS,cw=CW,s=S,w=W;
        cs -= s_m; p -= s*s_m;
        w_m = min(p/w,cw); cw -= w_m; p -= w*w_m;
        
        s_f = min(f/s,cs); cs -= s_f; f -= s*s_f;

        w_f = min(f/w,cw); cw -= w_f; f -= w*w_f;
        ckmax(ans,s_m+w_m+s_f+w_f);
    }
    ps(ans);
