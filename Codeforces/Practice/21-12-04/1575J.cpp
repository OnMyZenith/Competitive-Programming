...
void solve() {
    int n,m,k;re(n,m,k); vvi g(n,vi(m)); re(g); vi pos(k);re(pos);
    // dbg(g);
    each(i, pos){
        int res = y_combinator([&](auto self, int r, int c)->int{
            // dbg(r,c);
            // int new_r = r+ dr[g[r][c]];
            // int new_c =c+ dc[g[r][c]];
            if(r==n)return c;
            if(g[r][c]==1){g[r][c] = 2;return self(r,c+1);}
            if(g[r][c]==3){g[r][c] = 2;return self(r,c-1);}
            if(g[r][c]==2){return self(r+1,c);}
            
            // if(new_r==n)return new_c;
            // return self(new_r,new_c);
        })(0, i - 1);
        cout<<res+1<<" ";
    }
}
