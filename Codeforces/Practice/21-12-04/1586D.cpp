...
void f(int x){
    vb v(n); v[x]=1;
    cout<<"? "; f0r(i,n)cout<<(v[i]?"2":"1")<<" \n"[i==n-1];
    cout.flush();
}


void solve() {
    re(n);int cnt=n; tmp.ass(n,0);know.ass(n,0);fin.ass(n,0);
    bool fl=0; f(n-1);
    tmp[n-1]=1;
    int prevx = n-1;
    while(1) {
        ...
        if(x!=0&&x<prevx+1){
            ...
            f0r(i,n) if(tmp[i]){ tmp[i]=0;know[i]=1; fin[i]=cnt--; }
            fl=1;
            f0rd(i,n-1)if(!fin[int(i)]){fl=0;f(int(i));prevx = int(i);tmp[int(i)]=1;break;}
            if(fl){ cout<<"! "; f0r(i,n) cout<< fin[i]<<" \n"[i==n-1]; cout.flush(); return; }
        }
    }
}
