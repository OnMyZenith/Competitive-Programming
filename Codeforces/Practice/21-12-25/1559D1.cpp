...
void solve() {
    int n,m1,m2; re(n,m1,m2); union_find UF1(n), UF2(n);
    f0r(i,m1){int x,y; re(x,y); x--,y--;UF1.unite(x,y);}
    f0r(i,m2){int x,y; re(x,y); x--,y--;UF2.unite(x,y);}
    vpi res;
    f0r(i,n)f1r(j,i+1,n-1){if(UF1.find(i)!=UF1.find(j) && UF2.find(i)!=UF2.find(j)){
        UF1.unite(i,j);UF2.unite(i,j); res.pb({i+1,j+1});
    }}
    ps(sz(res));
    each(i,res) cout<<i.ff<<" "<<i.ss<<nl;
