...
struct Tree {
	int f(int a, int b, int pos) {return ((res[pos]=='0') ? 0 : a) + ((res[pos]=='1') ? 0 : b); }
	vector<int> s; int n;
	Tree(int n = 0, int def = 1) : s(2*n, def), n(n) {}
	int update(int pos) {
		while(pos){ s[pos] = f(s[pos * 2], s[pos * 2 + 1], pos);pos/=2;}
        return s[1];
	}
};

void solve() {
    int k;re(k);re(res); res+=' '; reverse(all(res)); int nq;re(nq); V<pair<int,char>> q(nq);re(q);
    Tree thing(1<<(k));
    f1rd(i,(1<<k)-1,1){thing.update((int)i);}
    each(i,q){
        res[(1<<k) - i.ff]=i.ss;
        ps(thing.update((1<<k) - i.ff));
    }
}
