...
void solve() {
    str s;re(s);
    int i=0,j=(int)sz(s)-1;
    if(sz(s)==1)goto aaa;
    while(i<sz(s)&&j>-1&&s[i]==s[j])i++,j--;
    if(j>i) {ps("YES");
        if(s[i]!='a'){ f0r(k,sz(s)){if(k==i+1)pr('a');pr(s[k]);}ps();return;}
        else { f0r(k,sz(s)){if(j==k)pr('a');pr(s[k]);}ps();return;}
    }else{
        aaa:
        bool f=1;
        each(c,s)if(c!='a')f=0;
        if(f){ps("NO");return;}
        else {ps("YES");pr(s);pr('a');ps();}
    }
}
