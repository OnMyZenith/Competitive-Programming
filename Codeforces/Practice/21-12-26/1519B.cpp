...
bool f(int n1, int m1, int k1){
    // dbg(n1,m1,k1);
    if(k1==0&&n1==n&&m1==m) return 1;
    if(n1>n||m1>m) return 0;
    if(k1<n*(m-m1)+(n-n1)*m1) return 0;
    return f(n1+1,m1,k1-m1)&&f(n1,m1+1,k1-n1);
}

void solve() {
    re(n,m,k); 
    if(k==n*(m-1)+(n-1)) ps("YES"); else ps("NO");
    // if(f(1,1,k)) ps("YES"); else ps("NO");
}
