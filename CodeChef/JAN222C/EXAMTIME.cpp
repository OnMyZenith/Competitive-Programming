...
void solve() {
    rv(3,a); rv(3,b); int sa = accumulate(all(a),0LL), sb = accumulate(all(b),0LL);
    if(sa>sb) {ps("DRAGON");return;}
    if(sa<sb) {ps("SLOTH");return;}

    if(a[0]>b[0]) {ps("DRAGON");return;}
    if(a[0]<b[0]) {ps("SLOTH");return;}



    if(a[1]>b[1]) {ps("DRAGON");return;}
    if(a[1]<b[1]) {ps("SLOTH");return;}
