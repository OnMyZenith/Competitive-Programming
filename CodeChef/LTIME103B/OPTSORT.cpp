...
tcT > struct MaxSegtree {
    ...
    static constexpr T NEUTRAL_VAL = 0LL; // Change this
...
vl a;
// void solve(){
//     int n; re(n); rv(n,a); vl b = a; sor(b); ll MX=0,MN=iINF;f0r(i,n){ckmin(MN,a[i]);ckmax(MX,a[i]);}
//     MinSegtree<ll> mn; MaxSegtree<ll> mx; mn.build(a); mx.build(a);
//     vb v(n,1);
//     f0r(i,n){
//         if(a[i]==b[i]){
//             v[i]=0;
...
void solve(){
    ...
    map<pl,int> m; f0r(i,n) m[{a[i],i}];
    int IDX=0; each(i,m) i.ss = IDX++;
    ...
    ll cost=0;
    for (int i = 0; i < n;){
        if(a[i]==a[idx[i]]){i++; continue;}
        ll mx=a[i], mn=a[i]; int mx_idx=m[{a[i],i}];
        while(i<n&&i<=mx_idx){
            ckmax(mx_idx,m[{a[i],i}]); ckmax(mx,a[i]); ckmin(mn,a[i]); i++;
        }
        // dbg(mx,mn,cost);
        cost+=(mx-mn);
        // dbg(cost);
...
// void solve(){
//     int n; re(n); a.rsz(n+1); for (int i = 1; i <= n; i++) cin>>a[i];

//     ll cost=0;
//     for (int i = 1; i <= n;){
//         ll mx=a[i], mn=a[i];
//         while(i<=n&&i<=mx){
//             ckmax(mx,a[i]); ckmin(mn,a[i]); i++;
//         }
//         // dbg(mx,mn,cost);
//         cost+=(mx-mn);
//         // dbg(cost);
...
//     f0r(i,n){
//         if(a[i]==b[i]){
//             cost -= (i==(n-1)?b[i]:b[i+1]) - (i?b[i-1]:b[i]);
//         }
//     }
//     ps(cost);
// }
...
// void solve() {
//     int n; re(n); rv(n,a); ll mn=iINF, mx=0; ll MX=0,MN=iINF;f0r(i,n){ckmin(MN,a[i]);ckmax(MX,a[i]);}
//     ll cost = 0;
//     for (int i = 0; i < n; ){
//         if(a[i]>mx) mx=a[i],mn =a[i],i++;
//         else{
//             ckmin(mn,a[i]);
//             while(i<n&&a[i]<=mx)ckmin(mn,a[i]),i++;
//             cost+=max(mx-mn,0LL);
//         }
//         dbg(mn,mx,i,cost);
//     }
