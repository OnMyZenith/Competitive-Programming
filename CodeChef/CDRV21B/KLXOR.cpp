...
void solve() {
    int n,k; re(n,k); str s; re(s); int cnt=0;
    vpi x(n,pi{0,0}); if(s[0]=='0')x[0].ff++; else x[0].ss++;
    f1r(i,1,n-1){ x[i].ff = x[i-1].ff + (s[i]=='0'); x[i].ss = x[i-1].ss + (s[i]=='1');}

    for (int i = 0; i < k; i++){
        int z = x[i+n-k].ff - (i?x[i-1].ff:0); int o = x[i+n-k].ss - (i?x[i-1].ss:0);
        // dbg(z,o);
        z%=2; o%=2;
        if(o)cnt++;
    }
