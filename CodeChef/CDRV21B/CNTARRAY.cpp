...
mi ncr(ll n, ll r){
    mi res = fact[n]*inv(fact[r]);
    res*=inv(fact[n-r]);
    return res;
}

void solve() {
    ll n,m; re(n,m); rv(n,a); if(n>m){ps(0);return;}
    ll cnt=0; f0r(i,n) if(a[i]<=m) cnt++;
    fact.rsz(m+1); fact[0] = 1; fact[1] = 1;
    f1r(i,2,m) fact[i] = fact[i-1]*i;

    mi res= fact[m]*inv(fact[m-n]);
    
    f1r(r,1,cnt){
        mi val = ncr(cnt,r)*fact[m-r];
        val*=inv(fact[m-n]);
        if(r&1) res-=val;
        else res+=val;
    }
