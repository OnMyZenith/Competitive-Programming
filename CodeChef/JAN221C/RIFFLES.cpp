...
void solve() {
    int n,k; re(n,k);
    vi v(n); iota(all(v),1); vi tmp = v;
    if(n==2) {ao(v,n); return;}
    vvi per(sqrt(2*n)+1,vi(n)); per[0] = v; per[1].clear();
    f0r(i,n){
        if((i+1)&1) per[1].pb(v[i]);
    }
    f0r(i,n){
        if((i)&1) per[1].pb(v[i]);
    }
    int f = 0;
    int curr=1;
    while(per[1][curr]-1!=1&&f<=k) curr = per[1][curr]-1, f++;
    f++;
    if(!f){ao(v,n);return;}
    // dbg(f,n);
    k%=f;
