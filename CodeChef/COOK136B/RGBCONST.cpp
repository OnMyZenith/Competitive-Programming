...
void solve() {
    int r,g,b; re(r,g,b); int n = r+g+b; if(b>1&&((!(r&&g))||(r+g<b))){ps(-1);return;}
    if(b==1){
        vi a(n); iota(all(a),1); str res = "B";
        f0r(i,r)res+='R'; f0r(i,g)res+='G'; 
        ps(res);
        f0r(i,n-1) cout<<"1 "<<i+2<<nl;
        return;
    }
    str res; f0r(i,b) res+='B'; f0r(i,r) res+='R'; f0r(i,g) res+='G'; ps(res);
    int i = 1, j = b + 1, k = b + r + 1;
    cout<<i++<<" "<<j<<nl;  // b1---r1
    cout<<i++<<" "<<k<<nl;  // b2---g1
    cout<<j<<" "<<k<<nl;    // r1---g1
    j++,k++;
    while(j-b<=r&&i<=b) cout<<j++<<" "<<i++<<nl;
    while(k-b-r<=g&&i<=b) cout<<k++<<" "<<i++<<nl;
    f1r(l,b+2,b+r) cout<<l<<" "<< b+r+1<<nl;
    f1r(l,b+r+2,b+r+g) cout<<l<<" "<< b+1<<nl;
