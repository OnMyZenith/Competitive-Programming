...
void solve() {
    ll n; re(n); rv(n,a); map<ll,vl> m; f0r(i,n)m[a[i]].pb(i); ll ans=0;
    each(i,m){
        if(i.ff>sz(i.ss))continue;
        ll l=0, r=n-1; 
        ll headnode=i.ff-1; ll tail = 0;
        while((headnode+1)!=sz(i.ss)){
            ll nn = (a[headnode+1]-1)-tail+1;
            ans+=
            headnode++;
            tail=a[headnode-i.ff];
        }
