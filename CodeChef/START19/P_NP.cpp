...
void solve() {
    str s1,s; re(s1); int n =sz(s1), NP=0,P=0, N=0,NN=0;
    f0r(i,n)if(i<n-1&&s1[i]=='N'&&s1[i+1]=='P')i++,NP++;
    else if(i<n-1&&s1[i]=='N'&&s1[i+1]=='N')i++,NN++;
    else if(s1[i]=='P')P++; else N++;
    int cost =iINF;
    if(NP==P+N)ckmin(cost,N);
    if(NP>P){
        int c=0;
        P+=N;c+=N;N=0;
