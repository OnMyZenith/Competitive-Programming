...
struct P{ int x;int y; void flipVert(){y=9-y;} void flipHor(){x=9-x;} };

void solve() {
    P k,r1,r2; cin>>k.x>>k.y; cin>>r1.x>>r1.y; cin>>r2.x>>r2.y;
    if(k.y==8) { k.flipVert();r1.flipVert();r2.flipVert(); }
    if(k.x==8) { k.flipHor();r1.flipHor();r2.flipHor(); }

    if(k.x==1&&(r1.x==2||r2.x==2)&&(r2.y!=r1.y)&&(!(abs(k.y-r1.y)==1||abs(k.y-r2.y)==1))){ ps("YES");return; }
    if(k.y==1&&(r1.y==2||r2.y==2)&&(r2.x!=r1.x)&&(!(abs(k.x-r1.x)==1||abs(k.x-r2.x)==1))){ ps("YES");return; }
