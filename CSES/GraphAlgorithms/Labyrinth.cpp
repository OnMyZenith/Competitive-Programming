...
pi way (char c){
    f0r(i,4){
        if(dir[i] == c) return {dr[(i+2)%4], dc[(i+2)%4]};
    }
    return {0,0};
}
...
void solve() {
    int n,m; re(n,m); vs g(n); re(g);
    auto valid = [&](int x, int y) -> bool{ return x>=0 && x<n && y>=0 && y<m;};
    pi beg, fin;
    f0r(i,n) f0r(j,m){
        if(g[i][j]=='A'){beg = {i,j};}
        if(g[i][j]=='B'){g[i][j] = '.'; fin = {i,j};}       
    }
    Q<pi> q; q.push(beg);
    while(!q.empty()){
        if(q.front()==fin) break;
        int r = q.front().ff; int c = q.front().ss; q.pop();
        f0r(k,4){
            int nr = r + dr[k], nc = c + dc[k];
            if(valid(nr,nc) && g[nr][nc]=='.'){g[nr][nc] = dir[k]; q.push({nr,nc});}
        }
    }

    str path;
    int r = fin.ff, c = fin.ss;
    if(g[r][c] == '.'){ps("NO");return;}
    while(g[r][c]!='A'){
        path += g[r][c]; pi tmp =  way(g[r][c]);
        r += tmp.ff; c += tmp.ss;
    }
    reverse(all(path));
    ps("YES");
