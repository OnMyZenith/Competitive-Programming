...
void solve() {
    int n; re(n); V<range> r(n); f0r(i,n){re(r[i].s,r[i].e); r[i].idx = i;};
    vi s(n), e(n); f0r(i,n) s[i] = r[i].s, e[i] = r[i].e;
    sor(s); sor(e); vpi res(n);
    // dbg(s,e);
    f0r(i,n){
        int endBeforeStart = upb(e, r[i].s);
        int endBeforeEnd = upb(e, r[i].e) - 1;
        int contained = endBeforeEnd - endBeforeStart;
        int startBeforeStart = upb(s, r[i].s) - 1;
        dbg(contained, startBeforeStart, endBeforeStart, endBeforeEnd);
        res[i] = {contained, lwb(s, r[i].s) - lwb(e, r[i].e) + contained};
    }
    f0r(i,n) cout << res[i].ff << " \n"[i==n-1];
    f0r(i,n) cout << res[i].ss << " \n"[i==n-1];
}
