...
void solve() {
    int n; re(n); vpi a(n); re(a); sort(all(a),[](pi a, pi b)->bool{if(a.ff==b.ff)return b.ss<a.ss; return a.ff<b.ff;})
    set<pi> r; each(i,a)r.ins({i.ss,i.ff});
    f0r(i,n){
        erase(r, {a[i].ss,a[i].ff});
        (*r.begin()).;
    }
