...
int main() {
    ...
    vector<int> __tmp(k);
    multiset<int> can_start_at(__tmp.begin(), __tmp.end());

    int ans = 0;
    for (auto &[end, start] : mov) {
        auto it = can_start_at.upper_bound(start);
        if(it != can_start_at.end()) {
            dbg(-start, end);
            ans++;
            can_start_at.erase(it);
            can_start_at.insert(-end);
        }
    }
