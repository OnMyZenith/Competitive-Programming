...
void solve() {
    int n; re(n); vvi p(2*n,vi(3));
    f0r(i,n){
        int s,e; re(s,e); p[2*i] = {s,0,i+1}; p[2*i+1] = {e,1,i+1};
    }
    sor(p);
    vi avail = {1}; vi allot(n+1); int curr = 1; avail.reserve(n);
    each(v,p){
        bool st = !v[1]; int id = v[2];
        if(st){
            if(avail.empty()) avail.pb(++curr);
            int room = avail.back(); avail.pop_back();
            allot[id] = room;
        }else{
            avail.pb(allot[id]);
        }
