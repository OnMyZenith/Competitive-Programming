...
mi dfs(int v, int lvl){
    ...
    auto it = mp.find(make_pair(v, lvl));
    if(it != mp.end()) return it -> second;

    if(v == 9) {
        mi res = dfs(8, lvl + 1) + dfs(9, lvl + 1);
        mp[{v, lvl}] = res;
        return res;
    }
    if(v == 1) {
        mi res = dfs(2, lvl + 1) + dfs(1, lvl + 1);
        mp[{v, lvl}] = res;
        return res;
    }
    mi res = dfs(v - 1, lvl + 1) + dfs(v, lvl + 1) + dfs(v + 1, lvl + 1);
    mp[{v, lvl}] = res;
    return res;
}
