...
void solve() {
    str s;re(s); ll n=sz(s);
    vl res, rem; ll cnt=1;
    ll j=-1;
    f1r(i,1,n-1){
        if(s[i]=='?'&&s[i-1]!='?') j = i;//0?
        else if(s[i]!='?'&&s[i-1]=='?'&&j!=-1) {//?0
            if((s[j-1]==s[i]&&((i-j)%2==0))||s[j-1]!=s[i]&&((i-j)%2==1)){res.pb(cnt);rem.pb(i-j); cnt=0;i=j;j=-1;}
            else j=-1;
        }
        if(s[i]!='?'&&s[i]==s[i-1]){res.pb(cnt); cnt=1;}//00 11
        else cnt++;
    }
    res.pb(cnt);
    ll fin=0;
    dbg(res,rem);
    each(i,res)fin+=i*(i+1)/2;
    each(i,rem)fin-=i*(i+1)/2;
    ps(fin);
