...
struct C{ ll t,movTo,currPos;};
C c[100007];
void solve() {
    ll n;re(n); f0r(i,n)cin>>c[i].t>>c[i].movTo;c[n].movTo=0;c[n].t=lINF;
    ll res=0, lastCommTime=-1,nextCommTime=0,nextPos=0, dir=0,startPos=0;
    f0r(i,n+1){
        if(nextCommTime<=c[i].t) c[i].currPos=nextPos,startPos= c[i].currPos, lastCommTime=c[i].t,nextPos=c[i].movTo, nextCommTime=c[i].t+abs(c[i].currPos-c[i].movTo), dir = abs(c[i].movTo-c[i].currPos)?(c[i].movTo-c[i].currPos)/abs(c[i].movTo-c[i].currPos):0;
        else {
            c[i].currPos=startPos+ (c[i].t - lastCommTime)*dir;
        }
    }
    f0r(i,n+1)dbg(c[i].t,c[i].currPos,c[i].movTo);
    f0r(i,n){
        if((c[i].movTo<=c[i+1].currPos&&c[i].movTo>=c[i].currPos)||(c[i].movTo>=c[i+1].currPos&&c[i].movTo<=c[i].currPos))res++;
    }
    ps(res);
}
