...
void solve() {
    int n,m; re(n,m); vi a(n*m), idx(n*m); re(a); iota(all(idx), 0);
    sort(all(idx),[&](int i, int j)->bool{if(a[i]==a[j]) return j < i; else return a[i]<a[j];});

    hash_map<int, int> mp;
    each(i, a) mp[i]++;
    dbg(idx);
    f0r(i, n) {
        int v = a[idx[i * m]], cnt = 0;
        f0r(j, m) {
            if (a[idx[j + i * m]] == v) cnt++;
            else
                break;
        }

        if (cnt < m) {
            int st = i * m + cnt - mp[v], end = i * m + cnt - 1;
            assert(a[idx[st]] == a[idx[end]]);
            assert(a[idx[st]] == v);
            dbg(st, end, cnt, v);
            vi b = idx;
            rotate(idx.begin() + st, idx.begin() + st + cnt, idx.begin() + end + 1);
            if (st == end) assert(b == idx);
        }
    }
    f0r(i,n){
