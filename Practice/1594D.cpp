...
void solve() {
    ...
    vi color(n,-1);
    int tot = 0, one = 0;
    auto poss = y_combinator([&](auto dfs, int v, int p, int col)->bool{
        color[v] = col;
        tot++; one += color[v];
        for(auto &[u,w]: g[v]){
            if(u==p) continue;
            if(color[u]==-1){
                if(!dfs(u,v,(w?int(!col):col))) return 0;;
            }else {
                int shouldB = (w?int(!col):col);
                if(color[u]!=shouldB) return 0;
            }
        }
        return 1;
    });
