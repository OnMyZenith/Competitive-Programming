...
void solve() {
    str s; re(s); hash_map<char,int> h; bool paired=1;
    each(i,s)h[i]++; int n=sz(s)/2; int cnt=0; int cnt1=0,cnt2=0;
    each(i,h){if(i.ss>=2)cnt++;else cnt1++;}

    // each(i,h){if(i.ss>=2)cnt++;else{ if(paired){!paired;}else {!paired;cnt++;}}}
    ps(cnt+cnt1/2);
