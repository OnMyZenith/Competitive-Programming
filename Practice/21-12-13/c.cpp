...
void solve() {
    int n; re(n); rv(n,a); sor(a); vvl minCost(n,vl(2*n+1,iINF));
    f1r(i,1,2*n)minCost[0][i]=abs(a[0]-i);
    f1r(i,1,n-1){
        f1r(k,1,2*n-1){
            f1r(j,1,2*n-1){
                if(k+j<=2*n) ckmin(minCost[i][k+j],minCost[i-1][k]+abs(a[i]-(k+j)));
            }
        }
    }
    ll ans=iINF; f0r(j,2*n) ckmin(ans,minCost[n-1][j]);
    ps(ans);
}
