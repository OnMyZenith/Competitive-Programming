...
struct splitmix64_hash {
    ...
    size_t operator()(P<uint64_t,uint64_t> x) const {
        static const uint64_t FIXED_RANDOM = chrono::steady_clock::now().time_since_epoch().count();
        return splitmix64(x.ff + FIXED_RANDOM) ^ splitmix64(x.ss + FIXED_RANDOM);
    }
...
void solve() {
    int n; re(n); str s; re(s); hash_map<pi,int> mp; int cd = 0;
    f0r(i,n) {
        if(s[i] == 'D') cd++;
        int g = gcd(cd,i+1-cd);
        int d = cd/g, k = (i+1-cd)/g;
        int r = ++mp[{d,k}];
        pr(r," ");
    }
    ps();
...
// void solve() {
//     int n; re(n); str s; re(s); vi pred(n), prek(n);
//     f0r(i, n) {
//         pred[i] = (i ? pred[i - 1] : 0);
//         prek[i] = (i ? prek[i - 1] : 0);
//         if (s[i] == 'D') pred[i]++;
...
//     }
//     f0r(i, n) {
//         int d = pred[i], k = prek[i];
//         if (d && k) {
//             int g = gcd(d,k);
//             if(g==1){pr(1," ");continue;}
//             int len = d/g + k/g, cnt = 0, cd = 0, ck = 0, l = 0;
//             for (int j = len-1; j <= i; j += len){
//                 cd = pred[j] - (l?pred[l-1]:0); ck = prek[j] - (l?prek[l-1]:0);
//                 int ng = gcd(cd,ck);
//                 if(cd/ng==d/g&&ck/ng==k/g){cd=0;ck=0;l = j+1; cnt++;}
//             }
//             if(cd==0&&ck==0){pr(cnt," ");}
//             else pr(1," ");
//         }else{
