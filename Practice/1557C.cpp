...
void solve() {
    int n,K; re(n,K);
    ps(y_combinator([&](auto self, int k, bool odd)->mi{
        mi ans = pow(mi(2),n-1);
        if(k==0) return 1;
        if(odd){
            // ans = pow(mi(2),n-1);
            // f0r(i,n-1) ans*=2; // ways of selecting even no. of things, for draw
            return (ans+1) * self(k-1,odd);
        }else{
            // f0r(i,n-1) ans*=2;
            --ans;
            mi win = pow(mi(2),(ll)(k-1)*n);
            // mi win = 1; f0r(i,k-1) f0r(j,n) win*=2;
            return win + (ans) * self(k-1,odd);
        }
    })(K,n&1));
}
