...
void solve() {
    int n,k; re(n,k); str s,t; re(s,t);
    vi fs(26),frt(26); sor(s); sor(t);
    for (int i = 0, j = 0; ;){
        if(i < n && j < n){
            if(s[i]==t[j]) i++,j++;
            else if(s[i]>t[j]) frt[t[j++]-'a']++;
            else fs[s[i++]-'a']++;
        }else{
            while(i<n)fs[s[i++]-'a']++;
            while(j<n)frt[t[j++]-'a']++;
    ...
    int cnts = 0, cntt = 0;
    f0r(i,26){
        if(fs[i]%k||frt[i]%k){ps("NO"); return;}
        cnts+=fs[i];cntt+=frt[i];
        if(cntt>cnts){ps("NO"); return;}
    }
    ps("YES");
