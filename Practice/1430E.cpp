...
void solve() {
    int n; str s, r; re(n,s); r = s; vvi f(26);
    f0r(i,n) f[s[i]-'a'].pb(i);
    reverse(all(r));
    dbg(s);
    dbg(r);
    int i = 0, j = 0; ll ans = 0; vb v(n);
    dbg(f); ord_set<ll> overlap;
    while(j<n){
        // while(v[i]) i++;
        ll idx = lwb(f[r[j]-'a'],i);
        auto it = overlap.lb(f[r[j]-'a'][idx]);
        ll skip = 0;
        if(it==overlap.end()) skip = sz(overlap);
        else {skip = (ll)overlap.order_of_key(*it);}
        assert(f[r[j]-'a'][idx] - (i+skip)>=0);
        ans += (ll)f[r[j]-'a'][idx] - (i+skip);
        // v[f[r[j]-'a'][idx]] = 1;
        overlap.ins(f[r[j]-'a'][idx]);
        f[r[j]-'a'][idx] = i-1;
        j++;
        dbg(f,ans);
    }
