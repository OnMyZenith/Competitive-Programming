...
void solve(){
    ...
    f1r(i,n/2+(1&n),n-1){
        if(s[i]!='0'){
            str r = findSum(s.substr(i),s.substr(0,i));
            dbg(r);
            if(res.length() > r.length()) res = r;
            else if(res.length() == r.length() && res>r) res = r;
            // ckmin(res, r);
            break;
        }
    }
...
// void solve() {
//     int n; str s; re(n,s);
//     auto add = y_combinator([](auto self, str a, str b, int carry = 0) -> string {
//         // if (a.empty()) return b;
//         // if (b.empty()) return a;
//         int r = (a.empty()?0:(a.back() - '0')) + (b.empty()?0:(b.back() - '0')) + carry;
//         return self(a.substr(0,sz(a)-1),b.substr(0,sz(b)-1), r%10) + char(r/10 + '0');
//     });
//     if (n & 1) ps(min(add(s.substr(0, sz(s) / 2), s.substr(sz(s) / 2)), add(s.substr(0, sz(s) / 2 + 1), s.substr(sz(s) / 2 + 1))));
//     else ps(add(s.substr(0, sz(s) / 2), s.substr(sz(s) / 2)));
// }
