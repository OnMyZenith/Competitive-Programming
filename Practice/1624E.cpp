...
void solve() {
    int n,m; re(n,m); vs N(n); re(N); str s; re(s);
    vpi mp2(m,pi{-1,-1}), mp3(m,pi{-1,-1});
    f0r(i,n)
        f0r(j,m-1){
            auto idx = N[i].find(s.substr(j,2));
            if(idx!=N[i].npos){
                mp2[j] = {i,idx};
            }
        }
    ...
    f0r(i,n)
        f0r(j,m-2){
            if(mp2[j].ff==-1) continue;
            auto idx = N[i].find(s.substr(j,3));
            if(idx!=N[i].npos){
                mp3[j] = {i,idx};
            }
        }
    ...
    lst[0] = 0;
    f1r(i,1,m){
        if(i>=2&&mp2[i-2].ff!=-1&&lst[i-2]!=-1)
            lst[i] = i - 2;
        if(i>=3&&mp3[i-3].ff!=-1&&lst[i-3]!=-1)
            lst[i] = i - 3;
    }

    if(lst[m]==-1){ps(-1);return;}
    vvi res;
    for (int i = m; i >= 2;){
        if(i>=2&&lst[i]==i-2){
            res.pb({mp2[i-2].ss + 1, mp2[i-2].ss + 2, mp2[i-2].ff+1});
            i-=2;
        } else if(i>=3&&lst[i]==i-3){
            res.pb({mp3[i-3].ss + 1, mp3[i-3].ss + 3, mp3[i-3].ff+1});
            i-=3;
        }
    }
    reverse(all(res));
