...
void solve() {
    int n; re(n); ai(a,n); vi sums(n); f0r(i,n) sums[i] = (i?sums[i-1]:0) + a[i];
    vi f1(n+1), f2(n+1); f0r(i,n) f1[a[i]]++;
    f0r(i,n){
        f1r(j,i+1,n-1){int x= sums[j] - (i?sums[i-1]:0); if(x <= n) f2[x]++;}
    }
    int ans = 0; f0r(i,n+1) ans+=f1[i]*(f1[i]&&f2[i]);
    ps(ans);
