...
struct AC {ll t, p;};
void solve() {
    ll n,k; re(n,k); V<AC> ac(k); f0r(i,k) re(ac[i].p); f0r(i,k) re(ac[i].t); vl l(n+2,lINF);
    sort(all(ac),[](AC a, AC b)->bool{return a.p<b.p;});
    ll j=0; f1r(i,1,n){ ckmin(l[i], l[i-1]+1); if(j<=k-1&&ac[j].p==i) ckmin(l[i], ac[j++].t);}
    j=k-1; f1rd(i,n,1){ ckmin(l[i], l[i+1]+1); if(j>=0&&ac[j].p==i) ckmin(l[i], ac[j--].t);}
    f1r(i,1,n)cout<<l[i]<<" \n"[i==n];
}
