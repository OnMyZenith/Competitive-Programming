...
void solve() {
    int n, L,R; re(n,L,R); vi l(n), r(n); int ans = 0;
    f0r(i,L){int x; re(x);x--; l[x]++;} f0r(i,R){int x; re(x);x--; r[x]++;}
    // dbg(l,r,ans);
    f0r(i,n) {while(l[i]&&r[i]){l[i]--; r[i]--;}}
    // dbg(l,r,ans);
    L=0;R=0; f0r(i,n) {L+=l[i]; R+=r[i];}
    // dbg(L,R,ans);
    ans+=min(L,R);
    if(L-R>=2){
        f0r(i,n) while(L-R>=2 && l[i]>=2) { l[i]-=2; ans++; L-=2;}
        ans+=(L-R);
    }else if(R-L>=2){
        swap(L,R); swap(l,r);
        f0r(i,n) while(L-R>=2 && l[i]>=2) { l[i]-=2; ans++; L-=2;}
