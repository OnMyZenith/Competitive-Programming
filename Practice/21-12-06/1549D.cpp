...
void solve() {
    ll n;re(n);vl a(n), b(n-1);re(a); f0r(i,n-1){ b[i]=abs(a[i+1]-a[i]);} while(pct((int)sz(b))!=1)b.pb(1);
    if(n==1){ps(1);return;}
    n = sz(b);
    Tree st(n);
    f0rd(i,n-1) st.update(i, b[i]);
    dbg(st.s);
    ll ans=0;int hi=-1;
    f0r(i,n){
        while(hi+1<n-1&&st.query(i,hi+2)>1)hi++;
        
        // ll lo=i,hi=n-1;
        // // lo--;
        // while(lo<hi){
        //     ll mid = (lo+hi+1)/2;
        //     if(st.query(i,mid+1)>1)lo=mid;
        //     else hi = mid-1;
        // }
        // dbg(lo);
        ckmax(ans, lo-i+1);
