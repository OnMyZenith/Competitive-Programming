...
void solve() {
    int n; re(n); rv(n,a); int ans = iINF;
    int g = 0; each(i,a) g = gcd(i,g);
    each(i,a) i/=g;
    auto cost = [&](int tar)->int{
        if(ok[tar]) return mem[tar];
        int curr = 0;
        for(auto i:a){
            // dbg(tar);
            int one = iINF;
            ...
            while(i){
                int t = i; int oo = dd;
                while(t<tar) t*=2, oo++;
                if(t==tar) ckmin(one,oo);
                i/=2; dd++;
            }
            // dbg(one,curr);
            if(one==iINF) return ok[tar] = 1, mem[tar] = iINF, iINF;
            curr+=one;
        }
        ok[tar] = 1;
        dbg(curr,tar);
        return mem[tar] = curr, curr;
    };

    for(auto ele:a){
        while(ele) {ckmin(ans, cost(ele)); ele/=2;}
    }
    ps(ans);
}
