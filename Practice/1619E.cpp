...
void solve() {
    int n; re(n); vi a(n); re(a); sor(a); vi f(n+1); each(i,a) f[i]++;
    vl preFreq(n+2); f0r(i,n+1) preFreq[i+1] = preFreq[i] + f[i];

    bool f1 = 0; ll fill = 0; vpi q;
    f0r(i,n+1){
        ll pop = f[i];
        if(f1 || preFreq[i] < i){ cout<< -1 << " \n"[i==n]; f1 = 1; continue;}

        if(i>=2)if(f[i-2]>=2) q.pb({i-2,f[i-2]});
        if(i && !f[i-1]){
            pi t = q.back(); q.pop_back(); if(--t.ss>=2) q.pb(t);
            fill += i - 1 - t.ff;
            // f0rd(j,i-2) if(f[j]>=2){f[j]--; fill+=(i-1)-j;break;}
        }
        cout<< pop + fill << " \n"[i==n];
    }
}
