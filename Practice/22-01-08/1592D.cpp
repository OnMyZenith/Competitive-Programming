...
void solve() {
    ...
    auto dfs = y_combinator([&](auto self, int v, int p)->void{
        pa[v] = p; f.pb(v); each(u,g[v]) if(u!=p) self(u,v);
        f.pb(v);
    });
    dfs(0,-1);
    int N = sz(f);
    ...
    auto q =[&](int l, int r)->int{
        set<int> s; f1r(i,l,r) s.ins(f[i]);
        if(sz(s)<2) return iINF;
        auto it1 = MP.find(s);
        if(it1!=MP.end()) return it1->second;
        cout<<"? "<<sz(s);
