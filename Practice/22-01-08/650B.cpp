...
void solve() {
    int n; ll a,b,t; re(n,a,b,t); str s; re(s);
    vl pre(n), suff(n);
    f1r(i,0,n-1) pre[i] = 1 + (i?pre[i-1]:0) + (i?a:0) + (s[i]=='w'?b:0);
    f1rd(i,n-1,0) suff[i] = 1 + (i<n-1?suff[i+1]:0) + a + (s[i]=='w'?b:0);
    reverse(all(suff));
    int mx = upb(pre,t);
    dbg(mx);
    f0r(i,n){
        ll x = t;
        x-=pre[i]+i*a; if(x<0) continue;
        int cnt = upb(suff,x);
        ckmax(mx,i+1+cnt);
    }
    f0r(i,n) {suff[i]+=1+(s[0]=='w'?b:0);pre[i]-=1+(s[0]=='w'?b:0);}

    f0r(i,n){
        ll x = t;
        x-=suff[i]+(i+1)*a; if(x<0) continue;
        int cnt = upb(pre,x);
        // if(cnt<1) continue;
        ckmax(mx,i+1+cnt);
    }
