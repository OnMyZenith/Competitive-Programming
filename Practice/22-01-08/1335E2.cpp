...
void solve() {
    int n; re(n); vi v(n); re(v); int N = *max_element(all(v));
    vvi cnt(N+1,vi(n)), pos(N+1);
    cnt[v[0]][0] = 1; pos[v[0]].pb(0);
    f1r(i,1,N){
        f1r(j,1,n-1){
            cnt[i][j] = cnt[i][j-1];
            if(v[j]==i) cnt[i][j]++, pos[i].pb(j);
        }
    }
    int mx = 0; f1r(i,1,N) ckmax(mx,cnt[i][n-1]);
    f1r(x,1,N){
        f1r(cntx,1,sz(pos[x])/2){// l is beginning of mid seg
            // dbg(pos[x],cntx);
            int l = pos[x][cntx-1]+1;
            if((cnt[x][n-1] - cntx)<0||(cnt[x][n-1] - cntx)>=sz(pos[x]))continue;
            // dbg((cnt[x][n-1] - cntx));
            int r = pos[x][(cnt[x][n-1] - cntx)]-1;
            if(l>r)continue;
            // dbg(l,r,cntx,x);
            // dbg(cnt[x]);
            int cnty = 0;
            f1r(y,1,N){
            // dbg(cnt[y]);
                ckmax(cnty, cnt[y][r] - cnt[y][l] + int(y==v[l]));
            }
            // dbg(cnty);
            ckmax(mx,2*cntx+cnty);
        }
    }
    ps(mx);
    // dbg(pos);
