...
void solve() {
    int n,m,x; re(n,m,x); rv(n,a); pqinc<pi> p; f0r(i,m) p.push({0,i}); vpi idx;
    f0r(i,n) {pi pp = p.top(); p.pop(); idx.pb({i,pp.ss}); p.push({(pp.ff+a[i]),pp.ss});}
    vi res; while(!p.empty()) {pi pp = p.top(); p.pop(); res.pb(pp.ff);}
    sor(res); if(res.back() - res.front()>x){ps("NO");return;}
    ps("YES"); f0r(i,n) cout<<idx[i].ss+1<<" \n"[i==n-1];
}
